local lib = {RainbowColorValue = 0, HueSelectionPosition = 0,Default=game.CoreGui}
...
local function removeUIElements(container, names)
    for _, element in pairs(container:GetChildren()) do
        if table.find(names, element.Name) then  -- Check if name exists in the list
            element:Destroy()
        end
    end
end
local uiElementsToRemove = {"DragUI", "VNT"}
removeUIElements(lib.Default, uiElementsToRemove)
removeUIElements(game.Players.LocalPlayer.PlayerGui, uiElementsToRemove)
removeUIElements(game.CoreGui, uiElementsToRemove)
local ui = Instance.new("ScreenGui")
ui.Name = "DragUI"
ui.Parent = lib.Default or game.CoreGui or game.Players.LocalPlayer.PlayerGui
...
function lib:CreateWindow(config)
    configUILibrary = config
    text = configUILibrary.Title.." - "..configUILibrary.SubTitle
    CloseBind = configUILibrary.MinimizeKey or Enum.KeyCode.RightControl
    ...
    local tabhold = {}
    function tabhold:AddTab(config)
        ...
        function tabcontent:AddButton(config)
            text = config.Title
        ...
        function tabcontent:AddToggle(a,config)
            local toggled = false
            local text,default = tostring(config.Title),tostring(config.Default)
            ...
            local Togglecva = {}
            function Togglecva:OnChanged(callback)
            coroutine.wrap(
                function()
                    while wait() do
                        FrameToggle3.BackgroundColor3 = PresetColor
        ...
        function tabcontent:AddSlider(args, args1)
            local dragging = false
            text, min, max, start, callback = args1.Title,args1.Min,args1.Max,args1.Default,args1.Callback
           ...
           Slider.BackgroundTransparency = 0.6
            ...
            local SlideCircle1 = Instance.new("ImageButton")
            SlideCircle1.Name = "SlideCircle1"
            SlideCircle1.Parent = Slider
            SlideCircle1.BackgroundColor3 = PresetColor
            SlideCircle1.BackgroundTransparency = 1.000
            SlideCircle1.Position = UDim2.new(0.8378126722, 0, 0, 10)
            SlideCircle1.Size = UDim2.new(0, 20, 0, 20)
            SlideCircle1.Image = "rbxassetid://10734933056"
            SlideCircle1.ImageColor3 = PresetColor
            ...
            local function resetSlider()
                local startPos = (start or 0) / max
                local resetPos = UDim2.new(startPos, -6, -1.30499995, 0)
                local resetSize = UDim2.new(startPos, 0, 0, 3)
            
                CurrentValueFrame:TweenSize(resetSize, "Out", "Sine", 0.4, true)
                SlideCircle:TweenPosition(resetPos, "Out", "Sine", 0.4, true)
            
                SliderValue.Text = tostring(start and math.floor((start / max) * (max - min) + min) or 0)
                pcall(callback, start or 0) -- Call the callback function with the start value
            end
            
                SlideCircle1.MouseButton1Click:Connect(resetSlider)
            coroutine.wrap(
                function()
                    while wait() do
                        CurrentValueFrame.BackgroundColor3 = PresetColor
        ...
        function tabcontent:AddDropdown(args,args1)
            text, list = args1.Title,args1.Values
            local droptog = false
            local framesize = 0
            local itemcount = 0
            ...
            local Dropdowngafgas = {}
            function Dropdowngafgas:OnChanged(callback)
            for i, v in next, list do
                ...
                Item.MouseButton1Click:Connect(
                    function()
                        print(v)
                        droptog = not droptog
                        DropdownTitle.Text = args1.Title .. " - " .. tostring(v)
                        pcall(callback, v)
                        Dropdown:TweenSize(
                            UDim2.new(0, 363, 0, 42),
                            Enum.EasingDirection.Out,
        ...
        function tabcontent:AddParagraph(config)
           for i,v in pairs(config) do
              warn(config)
           end
        end
        function tabcontent:AddInput(a,config)
            text, disapper, callback = config.Title,true,config.Callback
        ...
        Tab.CanvasSize = UDim2.new(0, 0, 0, TabLayout.AbsoluteContentSize.Y)
            local Textboxcc = {}
            function Textboxcc:OnChanged(callback)
            warn(callback)
        end
            return Textboxcc 
        end
        ...
        function tabcontent.Trash()
             local trash = {TabBtn,Tab}
            for i,v in pairs(trash) do
                v:Destroy()
            end
        end
        function tabcontent.Tf()
           local trash = {TabBtn,Tab}
           for i,v in pairs(trash) do
               v.Visible = false
           end
